...
var AppCachePlugin = (function() {
    ...
    function fsCachePlugin(startPath, output , outpath , excludePath) {
        var fs = require('fs');
        ...
        function findSync(startPath) {
            function finder(path) {
                var syncPath = path ? path.replace(/(\.\.\/)(.*)/, '$2') : path;
                try {
                    var files = fs.readdirSync(syncPath);
                } catch (error) {
                    return result
                }
                files.forEach((val, index) => {
                    var pathname = syncPath + '/' + val;
                    var stat = fs.statSync(pathname);
                    if (!stat.isDirectory()) {
                        if(outpath){
                            result.push(outpath + pathname.replace(excludePath,''));
                        }else{
                            result.push(output.replace(/(\.\.\/)(.*)/, '$1') + pathname);
                        }
                        // result.push(output.replace(/(\.\.\/)(.*)/, '$1') + pathname);                        
                        //result.push(path.replace(/(\.\.\/)(.*)/, '$1') + pathname.replace(root_path, '.'));
                    } else {
                        // result = result.concat(findSync(pathname));
                        finder(pathname)
                    }
                });
            }
            if (isArrayFn(startPath)) {
                startPath.forEach(function(val) {
                    cacheList.concat(finder(val))
                });
            } else {
                cacheList.concat(finder(startPath))
            }
    ...
    function AppCachePlugin() {
        ...
        var excludePath = _ref.excludePath;
        var cache = _ref.cache ? fsCachePlugin(_ref.cache, output , outpath , excludePath) : _ref.cache;
